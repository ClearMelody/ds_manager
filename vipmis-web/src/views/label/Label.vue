<template>
  <div>
    <div>
      ...
      <el-dialog
        title="新增分组"
        :visible.sync="addLabelDialog.dialogVisible"
      ...
      <el-dialog
        title="编辑分组"
        :visible.sync="editLabelDialog.dialogVisible"
        :close-on-click-modal="false"
        width="30%"
      >
      ...
      <el-dialog title="删除分组" :visible.sync="delLabelDialog.dialogVisible" width="30%">
        <el-row>确定删除该分组吗？</el-row>
        <span slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    methods: {
      ...
      del() {
        let _that = this;
        _that.loading = true;
        API.delLabel(_that.delLabelDialog.currentData)
          .then(() => {
            _that
              .listLabelsByPage()
              .then(() => {
      ...
      listLabelsByPage() {
        ...
        return new Promise((resolve, reject) => {
          API.listLabelsByPage(_that.queryParam)
            .then(result => {
              if (!result) {
                _that.loading = false;
                reject(result);
